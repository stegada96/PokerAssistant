...
class Hand169 {
  ...
  String label() {
    String f(int r) => r==14?"A":r==13?"K":r==12?"Q":r==11?"J":r==10?"T":"$r";
...
MatrixDecision decisionForPos(AppSettings s, Pos9Max pos) {
  ...
  final callBuffer = (pos.index >= Pos9Max.co.index) ? 10.0 : 7.0; // later pos can defend a bit more
  final raise = topPercentHands(open);
  final call = topPercentHands((open + callBuffer).clamp(0.0, 100.0)).difference(raise);
